...
init(){
	...
	this.paddle=this.root.paddle_mc;
	this.ball=this.root.ball_mc;
	this.paddle.origin={x:this.paddle.x,y:this.paddle.y};
	this.ball.origin={x:this.ball.x,y:this.ball.y};
	this.score_txt=this.root.score_txt;
	this.paddle.on("pressmove",function(evt){
...
const pt = this.ball.localToLocal(0, ballSize.height/2, this.paddle);
	if (this.paddle.hitTest(pt.x, pt.y)){
	this.ball.move.y = -this.ball.move.y;
